...
/* SOME CONSTANTS */
let endpoint01 = "https://yfn1heuoo9.execute-api.us-east-1.amazonaws.com/default/project4augastine"; // you must provide your Web Service Endpoint URL
let endpoint02 = "https://api.qrserver.com/v1/create-qr-code/?size=250x250&data="; // go qr API url
...
let viewItemController2 = (itemid) => {
	...
	$.ajax({
		...
		"success": (results) => {
			...
			const name = results[0]['name'];
			const description = results[0]['description'];
			const estimatedvalue = results[0]['estimatedvalue'];
	...
	$.ajax({
		...
		"success": (results)=>{
			...
			if (results.length > 0){
				...
				$('#div-menu').show();   //show the default page
